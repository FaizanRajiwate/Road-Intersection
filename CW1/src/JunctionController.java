...
public class JunctionController extends Thread {
	...
	private void checkPhase() throws InterruptedException {
		...
		// iterates through all phases, advancing the traffic light and verifying that
		// it is green.
		// If green enters the phase, takes the first car, compares it's crossing time
		...
		// the vehicle is removed from the list of vehicles waiting to cross and added
		// to the crossed vehicles.
		// the next car is checked.
		// For optimization purposes, if the phase is empty the phase is switched to the
		// next,
		// the traffic light is advanced to red.
		while (true) {
			for (Phases phase : phaseList) {
				TrafficController controller = phase.getTrafficController();// get the traffic controller object of the
																			// phases
				...
				Thread initialState = advanceTrafficState(controller); // create a thread object to advance the traffic
																		// light.
				initialState.start();
				initialState.join(); // join the thread, so the junction controller thread waits for the light to be
										// advanced
				...
				if (controller.isGreen()) {
					while (phaseDuration > 0) {
						try {
							...
							createdVehicles = model.getVehicleList(); // get the updated vehicle list to check for new
																		// vehicles.
							int index = createdVehicles.indexOf(currCar.getPlateNumber()); // check for index of car, as
																							// this will be it's
	...
	private synchronized void vehicleCrossing(Phases phase, Vehicles vehicle, int index,
		...
		file.writeToFile(
				vehicle.getPlateNumber() + " has crossed the intersection. " + "Distance: " + phase.getWaitingLength());
	...
	private synchronized void updateMovingSegmentTable(Vehicles vehicle) {
		...
		if (segment.equals("1")) {
			...
			model.addToS1WaitingLength(-1 * vehicle.getVehicleLength()); // subtract the car's length from waiting
																			// length for the segment
			model.addToS1WaitingTime(-1 * vehicle.getCrossingTime()); // subtract the car's waiting time from time at
																		// segment.
			model.updateTableModel(model.getStatsModel(), 0, 1, Integer.toString(model.getS1counter())); // updates
																											// segment
																											// table at
																											// specified
																											// row and
