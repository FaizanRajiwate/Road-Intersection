...
public class Main {
	...
	public static long timeElapsed(long startingTime) {
		// This function takes the starting time of the program, and calculates the
		// elapsed time since.
		// The input is the starting time, it returns the elapsed time which is also an
		// integer (long)
		long timeElapsed;
	...
	public static float averageWaiting(LinkedList<Phases> phaseList, float waitTime) {
		...
		// It receives as input a linkedlist comprising of various phases known as
		// phaselist and
	...
	public static LinkedList<Float> controlIntersections(LinkedList<Phases> phaseList, long executionTime) {
		// execution Time is the amount of time the program was kept running for before
		// being exited
		// phaseList is the list of phases available at the intersection
		// The program iterates through each phase in the order of phase 1, phase 1,
		// phase 3, phase 4 etc....
		// and checks the vehicles in for if they can move through in the respective
		// allocated phase time,
		// at the point where a vehicle cannot cross successfully, the active phase
		// becomes the nest available phase.
