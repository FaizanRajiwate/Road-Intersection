...
public class VehicleRNG extends Thread {
	...
	private synchronized void rngVehicleCreator(Helper helper)
		...
		helper.evaluateVehicleFile(vehicleDetails, phaseList); // evaluates the vehicle details to confirm that the
																// vehicle meets the rules
		Vehicles car = helper.createVehicle(vehicleDetails, phaseList); // creates the vehicle
		boolean sortedPhase = helper.findPhase(car, phaseList); // finds the phase of the vehicle and allocates it to
																// that phase
		if (sortedPhase) {
	...
	public void run() {
		while (true) {
			...
			} catch (NumberFormatException | PhaseException | InaccurateDataException | DuplicateIDException
